...
module.exports = {
   ...
   */
  async addProperty( { name }, auth ) {
    if ( this.propertyExists( auth.sub, name ) ) {
    ...
    const title  = [
      `${ monthNames[ date.getMonth() ] }_`,
      `${ date.getFullYear() }_${ name.toLowerCase() }`
    ].join('');
    ...
    return propertiesMap.addProperty( auth.sub, name, spreadsheetId );
  }
